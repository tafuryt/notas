...
def formatear_flex(df):
    ...
    df_resultado['VALOR'] = df['VALOR'].apply(
        lambda x: str(int(float(x))) if float(x).is_integer()
        else str(f"{float(x):.2f}").replace(".", ","))
...
if archivo:
    ...
    if df_raw.shape[1] != len(columnas_esperadas):
        ...
        try:
            total_valor = df_raw["VALOR"].str.replace(".", "", regex=False).str.replace(",", ".", regex=False).astype(float).sum()
