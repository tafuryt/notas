...
if archivo:
    ...
    try:
        df_raw = pd.read_csv(
            ...
            on_bad_lines='warn',
            encoding="utf-8-sig"  # <-- esta línea elimina el BOM
            )
    ...
    if df_raw.shape[1] != len(columnas_esperadas):
        ...
        df_raw = df_raw.fillna('')
         # ✅ Eliminar BOM solo si está presente en la primera celda del DataFrame
        df_raw.iloc[0, 0] = df_raw.iloc[0, 0].replace('\ufeff', '') if isinstance(df_raw.iloc[0, 0], str) else df_raw.iloc[0, 0]
